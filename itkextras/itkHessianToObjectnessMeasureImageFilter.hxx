  ...
  while ( !it.IsAtEnd() )
    ...
    if ( m_ObjectDimension < ImageDimension - 1 )
      ...
      if ( vcl_fabs(rADenominatorBase) > 0.0 )
        ...
        if ( vcl_fabs(m_Alpha) > 0.0 )
          ...
          objectnessMeasure *= 1.0 - vcl_exp( -0.5 * itk::Math::sqr (rA) / itk::Math::sqr (m_Alpha) );
          }
        }
      else
        {
    ...
    if ( m_ObjectDimension > 0 )
      ...
      if ( vcl_fabs(rBDenominatorBase) > 0.0 && vcl_fabs(m_Beta) > 0.0 )
        ...
        objectnessMeasure *= vcl_exp( -0.5 * itk::Math::sqr (rB) / itk::Math::sqr (m_Beta) );
        }
      else
        {
        objectnessMeasure = 0.0;
    ...
    if ( vcl_fabs(m_Gamma) > 0.0 )
      ...
      objectnessMeasure *= 1.0 - vcl_exp( -0.5 * frobeniusNormSquared / itk::Math::sqr (m_Gamma) );
      }
